...
printFrameSync (dsd_opts * opts, dsd_state * state, char *frametype, int offset, char *modulation)
{
  ...
  if (opts->verbose > 0)
    {
      sprintf (msg,"Sync: %s ", frametype);
      strcat(state->msgbuf,msg);
    }
  if (opts->verbose > 2)
    {
      sprintf (msg,"o: %4i ", offset);
      strcat(state->msgbuf,msg);
    }
  if (opts->verbose > 1)
    {
      sprintf (msg,"mod: %s ", modulation);
      strcat(state->msgbuf,msg);
    }
  if (opts->verbose > 2)
    {
      sprintf (msg, "g: %f ", state->aout_gain);
      strcat(state->msgbuf,msg);
    }
}
                  ...
                  if (state->symbolcnt > (4800 / opts->scoperate))
                    {
                      ...
                      printf ("Frame Type:    %s        Talkgroup:            %7ld\n", state->ftype, state->lasttg);
                      printf ("Frame Subtype: %s       Source:          %12ld\n", state->fsubtype, state->lastsrc);
